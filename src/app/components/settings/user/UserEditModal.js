...
const UserEditModal = (props) => {
    ...
    const onClickSaveUserButton = async () => {
        ...
        const response = await UserAPI.updateUserItem(
            selectedUserInfo.userId,
            userName,
            isActiveType);
        if (response.isSuccess) {
            NonoToast.success("참여자 정보를 수정 하였습니다.");
            onClickCloseButton();
            setUserName("");

        } else {
            NonoToast.error("참여자 정보 수정에 실패하였습니다.");
        }
    }

    return (
        <div className="userEditModalPage">
            <div className="companyNewTitleSection">
                <span className="userEditModalTitle">참여자 정보 수정</span>
